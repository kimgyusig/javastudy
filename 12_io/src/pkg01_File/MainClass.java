...
public class MainClass {
  ...
  public static void method1() {
    ...
    // File 객체 생성
    dir = new File("\\storage");                // c: 생략가능          // parent : dir   child : file
  ...
  public static void method3() {
     ...
     * 5. lastModified() : 최종수정일을 log 타입으로 반환  -> 날짜가 long 타입이면 timeStamp
    ...
    for(File file : files) {
     ...
     // 크기
     long byteSize = file.isDirectory() ? 0 : file.length();// byte 크기 반환
     long kbSize = (byteSize / 1024) + (byteSize % 1024 != 0 ? 1 :0);
     if(byteSize != 0)
     builder.append(String.format("%10s", new DecimalFormat("#,##0").format(byteSize) + "KB"));
  ...
  public static void main(String[] args) {
    ...
    practice2(new File("\\2024\\01\\22"));   // practic2 매개변수가 받는다 File dir
  ...
  public static void practice2(File dir) {
    ...
    File parent = dir.getParentFile();  //22일의부모인 1월을 parent 로 저장후 반복1월의부모저장 
    if(parent == null) {
      return;  // 메소드를 그만호출한다
    }
    // 현재 디렉터리 삭제
    System.out.println(dir.getPath() + "삭제하였습니다");      // 삭제
    dir.delete();
